...
let _make_percent = function(V) {
  return V < 0.01 ? '< 1%' : Math.round(100*V) + '%';
};

StephEtaPlotter.prototype._plot_etas = function(svg, eta, pop_labels, samp_labels, col_widths, col_spacing, pop_colours, col_label_height, y_offset) {
  let self = this;
  let K = eta.length;
  let S = eta[0].length;
...
StephEtaPlotter.prototype._plot_diversity_indices = function(svg, cdi, cmdi, y_offset, col_widths, col_spacing, colour) {
  let self = this;
...
StephEtaPlotter.prototype._plot_discord = function(svg, discord_pairs, y_offset, col_widths, col_spacing, colour) {
  let self = this;
  let cum_col_width = this._calc_cum(col_widths);
  let cum_col_spacing = this._calc_cum(col_spacing);
...
StephEtaPlotter.prototype._make_diversity_idx_legend = function(svg, colour, x_offset, y_offset) {
  let scale = d3.scaleSequential([0, 100], colour);
...
StephEtaPlotter.prototype._add_yaxis = function(svg, height, yoffset) {
...
StephEtaPlotter.prototype.plot = function(eta, cdi, cmdi, discord, samp_labels, container, remove_small_pops=false, remove_normal=false) {
  let self = this;
  ...
  let discord_y_offset = 2*this._diversity_idx_height + 2*this._diversity_idx_spacing + col_label_height;
  let bar_y_offset = discord_y_offset + this._discord_height + this._discord_spacing;
